...
Some Codex App hosts reject the combination of “full-context fork + explicitly specified role”. This is not a routing failure or a security issue. It is a subagent transport limit. When it appears, use the explicit role task with only the required compact context written into the task, and do not retry the same full-context fork plus role override combination. `executionAdapter.spawnInvocation` exposes the safe call shape: explicit role, compact required context in the task body, and `fullContextFork: false`.
