...
import {
  DISPLAY_BOARD_SCHEMA_VERSION,
  MANAGED_INTERNAL_LEAK_PATTERN,
  MANAGED_SECRET_LEAK_PATTERN,
  validateManagedPlanContract,
} from "./managed-plan-contract.mjs";
import {
  DEFAULT_HIGH_RISK_RULES,
  DEFAULT_TASK_KIND_POLICY,
} from "./task-kind-policy.mjs";
import {
  applyProjectSignalsToTaskKind,
  projectBoostForAgent,
  projectPreferredAgentNames,
...
function routeTask(task, options = {}) {
  ...
  let ranked = allAgents.agents
    .map((agent) => {
      ...
      const projectBoost = projectBoostForAgent(agent, task, taskKind, projectSignals);
  ...
  const taskKindPreferred = preferredAgentsForTaskKind(taskKind)
    ...
    .filter(Boolean);
  const projectPreferred = projectPreferredAgentNames(task, taskKind, projectSignals, { hasExplicitSecurityRisk: hasExplicitSecurityRiskSignal(task) })
    .map((name) => allAgents.agents.find((agent) => agentMatches(agent, name)))
    .filter(Boolean);
  if (projectPreferred.length) {
...
function displayBoardFor(plan) {
  ...
  const offlinePreview = /offline/i.test(`${plan.nextAction?.reason || ""} ${plan.delegationReadiness?.reason || ""} ${plan.clarificationQuestion || ""}`);
  ...
  const boardState = offlinePreview
    ? "离线预览，需父级复核"
    : plan.nextAction?.type === "ask-clarification"
    ? "需要先问一个问题"
  ...
  return {
    ...
    safetyPanel: {
      ...
      blockedChecks: [
        ...(offlinePreview ? ["离线预览不能作为自动派发依据"] : []),
        ...blockedChecks,
      ].slice(0, maxSafetyItems).map((item) => displayText(item, 120)),
      requiresParentReview,
...
function compactManagedPlanForProfile(plan, profile = "compact") {
  ...
  return {
    ...
    executionAdapter: plan.executionAdapter ? {
      ...
      traceSafeFields: (plan.executionAdapter.traceSafeFields || []).filter((field) => ["mode", "bridgeRole", "providerTransport", "promptInjectionRequired", "spawnInvocation"].includes(field)),
      codexAppSpawnConstraint: displayText(plan.executionAdapter.codexAppSpawnConstraint, 130),
      spawnGuidance: displayText(plan.executionAdapter.spawnGuidance, 120),
      spawnInvocation: plan.executionAdapter.spawnInvocation ? {
        ...
        forbiddenShapes: (plan.executionAdapter.spawnInvocation.forbiddenShapes || []).slice(0, 3).map((shape) => ({
          ...
          reason: displayText(shape.reason, 90),
        })),
        requiredTaskContext: (plan.executionAdapter.spawnInvocation.requiredTaskContext || []).slice(0, profile === "app" ? 5 : 3),
        omitFromToolCall: (plan.executionAdapter.spawnInvocation.omitFromToolCall || []).slice(0, 3),
    ...
    agentWorkPlan: (plan.agentWorkPlan || [])
      .filter((card) => profile === "app" || card.agent || card.rosterRole !== "implementer")
      .map((card) => ({
      rosterRole: card.rosterRole,
      agent: card.agent,
      agentProvider: card.agentProvider,
      responsibility: displayText(card.responsibility, profile === "app" ? 120 : 80),
      permission: card.permission,
      canRunInParallel: card.canRunInParallel,
      inputs: (card.inputs || []).slice(0, profile === "app" ? 2 : 1).map((item) => displayText(item, 80)),
      outputs: (card.outputs || []).slice(0, profile === "app" ? 2 : 1).map((item) => displayText(item, 80)),
      acceptance: (card.acceptance || []).slice(0, profile === "app" ? 2 : 1).map((item) => displayText(item, 80)),
      handoffTo: (card.handoffTo || []).slice(0, profile === "app" ? 2 : 1),
    })),
...
function pluginMirrorSyncHealth() {
  ...
  const pairs = [
    ...
    ["managed-plan-contract", path.join(projectRoot, "subagents", "managed-plan-contract.mjs"), path.join(projectRoot, "plugins/codex-subagent-router/scripts/subagents/managed-plan-contract.mjs")],
    ["semantic-routing", path.join(projectRoot, "subagents", "semantic-routing.mjs"), path.join(projectRoot, "plugins/codex-subagent-router/scripts/subagents/semantic-routing.mjs")],
    ["task-kind-policy", path.join(projectRoot, "subagents", "task-kind-policy.mjs"), path.join(projectRoot, "plugins/codex-subagent-router/scripts/subagents/task-kind-policy.mjs")],
...
function runAppBoardTests() {
  ...
  assert(text.includes("离线预览"), "managed --offline --profile app text should clearly label offline preview mode");
  ...
  const sensitiveTask = "开启子代理，审查 credential 工具，样例 access_token=abc123SECRET456、api_key=key_live_789、Bearer ghp_exampleSECRET000 和 user@example.com，不要输出 token";
  ...
  assert(!/abc123SECRET456|key_live_789|ghp_exampleSECRET000|user@example\.com/.test(sensitiveJson), "compact/app managed plans must redact secret-shaped values recursively");
